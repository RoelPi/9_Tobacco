...
dt <- data.table(dt[,.(maand,adValoremTabak,bijzonderAdValoremTabak,specifiekeAccijnsTabak,bijzonderSpecifiekeAccijnsTabak,minimumTabak,indexTabak)])


# Bereken minimum kleinhandelsprijs & basisprijs op basis van minimumlasten uit wetgeving
dt[,minPrijs:=(minimumTabak-specifiekeAccijnsTabak-bijzonderSpecifiekeAccijnsTabak)/1000*50/(adValoremTabak+bijzonderAdValoremTabak)]


# Bereken gemiddelde kleinhandelsprijs van pak sigaretten op basis van index
indexNu <- dt[maand == maandNu]$indexTabak
dt[,avgPrijs:=indexTabak/indexNu*prijs]

# Basisprijs berekenen op basis van geëxtrapoleerde huidige prijs - lasten, dit is nodig om voorspelling te maken
dt[,baseAvgPrijs:=(1-adValoremTabak-bijzonderAdValoremTabak)*avgPrijs/1.21-(specifiekeAccijnsTabak+bijzonderSpecifiekeAccijnsTabak)/1000*50]
...
toPredict <- toPredict[,avgPrijs:=(baseAvgPrijs+(specifiekeAccijnsTabak+bijzonderSpecifiekeAccijnsTabak)/1000*50)*1.21/(1-adValoremTabak-bijzonderAdValoremTabak)]

# Samenvoegen waargenomen data en voorspelling
dt <- data.table(rbind(dt,toPredict))

# Bereken prijsonafhankelijke prijscomponenten
dt[,c('sa','bsa'):=list(specifiekeAccijnsTabak/1000*19,bijzonderSpecifiekeAccijnsTabak/1000*50)]

# Bereken prijscomponenten voor gemiddelde prijs
# Base prijs reeds berekend
dt[,c('avAvgPrijs','bavAvgPrijs','btwAvgPrijs'):=list(avgPrijs*adValoremTabak,avgPrijs*bijzonderAdValoremTabak,(baseAvgPrijs+sa+bsa)*0.21)]

# Bereken prijscomponenten voor minimumprijs
dt[,baseMinPrijs:=(1-adValoremTabak-bijzonderAdValoremTabak)*minPrijs/1.21-(specifiekeAccijnsTabak+bijzonderSpecifiekeAccijnsTabak)/1000*50]
dt[,c('avMinPrijs','bavMinPrijs','btwMinPrijs'):=list(minPrijs*adValoremTabak,minPrijs*bijzonderAdValoremTabak,(baseMinPrijs+sa+bsa)*0.21)]
